...
| [nlpm](https://github.com/xiaolai/nlpm-for-claude) | NLPM — scan, score, check, fix, test, security-scan, and trend-track NL artifacts with focused agents, working hooks, and the 50 Rules | 0.7.20 |
...
| [ui-tokenize](https://github.com/xiaolai/ui-tokenize) | UI Tokenize — block hardcoded UI values; rewrite-first PreToolUse hook corrects literals to design-token references on the way to disk; configurable `strict` (deny on uncertain) or `advisory` (PostToolUse findings); per-project `surfaces` allowlist narrows scanning | 0.3.0 |
