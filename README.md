...
|--------|-------------|---------|
| [cc-suite](https://github.com/xiaolai/cc-suite) | CC Suite — one plugin to bridge and delegate across Claude Code, Codex CLI, and Gemini CLI: single-source AGENTS.md, shared skills, mirrored hooks and MCP servers, full Claude↔Codex bidirectional delegation. Supersedes cc-bridge and codex-toolkit. | 0.2.12 |
...
| [nlpm](https://github.com/xiaolai/nlpm-for-claude) | NLPM — scan, score, check, fix, test, security-scan, and trend-track NL artifacts via focused agents and the 50 Rules; v0.8.0+ adds a standalone Python validator and a shields.io "Validated by NLPM" badge for pre-commit / CI that catches manifest-vs-disk inconsistency; v0.8.5 adds multi-plugin monorepo support; v0.8.15–v0.8.16 add a two-stage rubric-vs-findings drift detector; v0.8.17 adds an exemplar pipeline; v0.8.18 adds an auto-generated exemplar gallery and a human-gated PR pipeline that cites exemplars from the rules file; v0.8.19 auto-persists trend snapshots from /init and every /score; v0.8.20 skips the codex/ subtree on dual-platform plugins; v0.8.21 stops promoting clean-no-bugs audits into the contribute lane and surfaces drift columns in the daily report; v0.8.22 skips top-level `templates/` directories during scan; v0.8.23 ships audit-fix corrections (2 HIGH bugs in render-repo-report and diff-findings), the BigQuery velocity-harvest discovery channel, the vendor-default filter, and batch-process suppression for low-landing-rate rules | 0.8.23 |
| [claude-english-buddy](https://github.com/xiaolai/claude-english-buddy-for-claude) | English language coach — auto-corrects prompts, translates non-English, tracks improvement over time | 0.5.0 |
...
| [eou-foundry](https://github.com/xiaolai/eou-foundry) | EOU Foundry — turn messy workflows into auditable Executable Operating Units with recursive governance (10 skills: candidate generation, audit, specify, refactor, promote, foundry-wide audit, ECP authoring, init scaffolding). v0.6.0 ships the lifecycle/evidence triangle + candidate-set schema + active-EOU trace gate (hard-cut). | 0.6.0 | |
